...
async function checkColumnsExist(pool) {
    try {
    ...
    `);
        ...
        const acudientesComplete = ['frequency_ratings5', 'frequency_ratings6', 'frequency_ratings7'].every(col => acudientesColumns.includes(col));
        const estudiantesComplete = ['frequency_ratings5', 'frequency_ratings6', 'frequency_ratings7'].every(col => estudiantesColumns.includes(col));
        const docentesComplete = ['frequency_ratings6', 'frequency_ratings7', 'frequency_ratings8'].every(col => docentesColumns.includes(col));
        // Check if new columns already exist
        const acudientesNewColumnsExist = ['Comunicacion', 'Practicas_Pedagogicas', 'Convivencia'].every(col => acudientesColumns.includes(col));
        const estudiantesNewColumnsExist = ['Comunicacion', 'Practicas_Pedagogicas', 'Convivencia'].every(col => estudiantesColumns.includes(col));
        const docentesNewColumnsExist = ['Comunicacion', 'Practicas_Pedagogicas', 'Convivencia'].every(col => docentesColumns.includes(col));
...
async function createNewColumns(pool) {
...
async function migrateColumnNames() {
    ...
    try {
        ...
        const confirmed = await promptForConfirmation('WARNING: This will rename columns in the database. This operation cannot be undone. Are you sure you want to proceed?');
        if (!confirmed) {
